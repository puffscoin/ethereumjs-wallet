...
A lightweight PUFFScoin wallet implementation. At the moment it supports key creation and conversion between various formats.

It is complemented by the following packages:
- [puffscoinjs-tx](https://github.com/puffscoin/puffscoinjs-tx) to sign transactions
- [puffscoinjs-icap](https://github.com/puffscoin/puffscoinjs-icap) to manipulate ICAP addresses
- [store.js](https://github.com/marcuswestin/store.js) to use browser storage

Motivations are:
- be lightweight
- work in a browser
- use a single, maintained version of crypto library (and that should be in line with `puffscoinjs-util` and `puffscoinjs-tx`)
...
Note: `getPublicKey()` only returns uncompressed PUFFScoin-style public keys.

## HD Wallet API

To use BIP32 HD wallets, first include the `hdkey` submodule:

`var hdkey = require('puffscoinjs-wallet/hdkey')`
...
```js
const WalletSubprovider = require('puffscoinjs-wallet/provider-engine')
