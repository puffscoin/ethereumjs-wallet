...
Thirdparty.fromEtherCamp = function (passphrase) {
  return new Wallet(puffsUtil.keccak256(Buffer.from(passphrase)))
}
...
Thirdparty.fromKryptoKit = function (entropy, password) {
  ...
  var privKey
  if (type === 'd') {
    privKey = puffsUtil.sha256(entropy)
  } else if (type === 'q') {
    if (typeof password !== 'string') {
      throw new Error('Password required')
    }

    var encryptedSeed = puffsUtil.sha256(Buffer.from(entropy.slice(0, 30)))
    ...
    */
    ...
    if (checksum.length > 0) {
      if (checksum !== puffsUtil.sha256(puffsUtil.sha256(privKey)).slice(0, 8).toString('hex')) {
